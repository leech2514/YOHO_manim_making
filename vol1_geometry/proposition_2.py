...
"""
    命题I.2：从一个给定的点可以引一条线段等于已知线段。

    迁移自 _2024/v1_Geometry_Fundamentals/PropositionI_2.py。
    find_intersection_points / find_line_circle_intersection 已抽至
    common.math_utils.MathUtils；show_proof 因依赖 self.play，仍保留为场景方法。
"""
...
class VolumeIPropositionII(Scene):
    def construct(self):
        ...
        if len(intersection_with_BF) > 0:
            ...
            # 找到circle4和线段DE的交点
            intersection_with_DE = MathUtils.find_line_circle_intersection(intersection_points[0], extended_dot_E, intersection_points[0], np.linalg.norm(intersection_points[0] - intersection_with_BF[0]))
