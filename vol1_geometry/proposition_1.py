...
"""
    ...
    设：AB为已知的线段。
    要求：以线段AB为边建立一个等边三角形，以A为圆心、AB为半径作圆BCD；
          再以B为圆心、以BA为半径作圆ACE；两圆相交于C点，连接CA、CB。

    迁移自 _2024/v1_Geometry_Fundamentals/PropositionI_1.py。
    find_intersection_points 已抽至 common.math_utils.MathUtils；
    show_proof / move_scene_elements 因依赖 self.play / self.mobjects，仍保留为场景方法。
"""
