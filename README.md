...
- **关于执行命令**：
  ...
  - **关于命令使用说明**：
   >   `manim -p -ql first.py SquareToCircle` ： 运行first.py文件中的SquareToCircle类<br>
       `-p` ： 表示渲染完毕后自动预览动画<br>
       `-ql` ： 表示低质量（’quality low’）渲染动画，速度快但是画质不好，视频分辨率和帧率较低;480p分辨率<br>
       `-qh` ： 高质量渲染（’quality high’）渲染动画，速度慢但是画质好，视频分辨率和帧率较高;720p分辨率<br>
       `-qm` ： 中质量渲染（’quality medium’）渲染动画，速度一般，画质一般，视频分辨率和帧率适中。<br>
       first.py ： 具体文件名<br>
       class_name ： 具体命题名称<br>
